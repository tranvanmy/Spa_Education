...
<script>
    ...
    export default {
        name: 'AdminResearchDevelopment',

        beforeCreate() {
            Helper.changeTitleAdminPage(this.$i18n.t('textManageResearchDevelopment'))
            this.$store.dispatch('actionFetchResearchDevelopments', { vue: this })
        },
        ...
        methods: {
            clickAddNewItem() {
                return this.$router.push({ path: '/research-developments/add' })
            },

            clickEditItem(data) {
                return this.$router.push({ path: `/research-developments/edit/${data.id}` })
            },
            ...
            async clickDeleteItem(id) {
                return id
                    ...
                    && this.$store.dispatch('actionResearchDevelopmentDelete', { vue: this, id })
            },

            changePage(page) {
                this.$store.dispatch('actionResearchDevelopmentSetPage', { page })
            },

            handleChangeValueFilter(value) {
                return this.$store.dispatch('actionResearchDevelopmentSetFilter', { value: value.trim() })
            },
        },
        ...
        computed: {
            currentPage: {
                get() {
                    return this.$store.state.storeAdminResearchDevelopment.currentPage
                },
                set(value) {}
            },

            valueFilter() {
                return this.$store.state.storeAdminResearchDevelopment.valueFilter
            },
            ...
            items() {
                return this.$store.state.storeAdminResearchDevelopment.listFetch
            },
        },
    }
</script>
